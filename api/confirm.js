...
// 서버 가격표 — 클라이언트가 보낸 금액을 믿지 않고 템플릿→등급→가격으로 검증
function priceOf(templateId) {
  const t = catalog.templates.find((x) => x.id === templateId);
  return t ? catalog.tiers[t.tier].price : null;
}
...
async function confirmHandler(req, res, deps = {}) {
  ...
  const price = priceOf(order && order.template);
  if (!price) return res.status(400).json({ error: "unknown template" });
